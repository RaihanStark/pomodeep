...
<div class="w-full">
  <div class="mx-auto rounded-md" style="background-color:#bf5a5a; padding:24px 28px 32px; max-width:640px;">
...
<style>
  .tab { background-color:#b65454; color:rgba(255,255,255,.9); padding:6px 10px; border-radius:6px; transition:background-color .2s ease, transform .12s ease; }
  .tab:hover { background-color:#ad4f4f; }
  .tab:active { transform:translateY(1px); }
  .active-tab { background-color:#a64646 !important; }

  .timer { color:#fff; text-align:center; font-weight:800; font-size:120px; line-height:1; letter-spacing:2px; margin-top:12px; user-select:none; transition:transform .14s ease; }
  .pulse { transform:scale(1.02); }

  .cta { background:#fff; color:#a64646; font-weight:700; letter-spacing:1px; padding:14px 52px; border-radius:8px; box-shadow:0 3px 0 #c84f4f; transition:transform .12s ease, box-shadow .12s ease, opacity .2s ease; }
